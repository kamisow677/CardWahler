...
class RestResponseEntityExceptionHandler : ResponseEntityExceptionHandler() {
    ...
    @ExceptionHandler(value = [Exception::class, JdbcUpdateAffectedIncorrectNumberOfRowsException::class])
    protected fun handlJdbcUpdateAffectedIncorrectNumberOfRowsException(
            ex: RuntimeException?, request: WebRequest?): ResponseEntity<Any> {
        val bodyOfResponse = "This should be application specific"
        return handleExceptionInternal(ex, bodyOfResponse,
                HttpHeaders(), HttpStatus.CONFLICT, request)
